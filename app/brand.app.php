...
class BrandApp extends MallbaseApp
{
    function index()
    {
        ...
        foreach($recommended_stores as $k=>$v)
        {
        	$recommended_stores[$k]['store_logo'] = SITE_URL."/".$v['store_logo'];
        }
        $this->assign('recommended_stores', $recommended_stores);
        $recommended_brands = $this->_recommended_brands(10);
        foreach($recommended_brands as $k=>$v)
        {
        	$recommended_brands[$k]['brand_logo'] = SITE_URL."/".$v['brand_logo'];
        }
        ...
        $this->assign('image_url',SITE_URL);
