...
class My_qaApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        $this->display('storeadmin.myqa.index.html');
    }
    function reply()
    {
        if (!IS_POST)
    ...
    function drop()
    {
        ...
        if (!$id)
        {
            $this->show_storeadmin_warning('no_qa_to_drop');
            return;
        }
        $ids = explode(',', $id);
        if (!$this->my_qa_mod->drop($ids))
        {
            $this->show_storeadmin_warning('drop_failed');
            return;
        }
        $this->show_storeadmin_message('drop_successful');
    }
    //三级菜单:
