...
class ArticleApp extends MallbaseApp
{
    ...
    function index()
    {
        ...
        $this->assign('navs', $this->_get_navs());
    	 /* 处理cate_id */
        $cate_id_now = !empty($_GET['cate_id'])? intval($_GET['cate_id']) : $this->_ACC[ACC_ABOUT]; //如果cate_id为空则默认显示关于我们
        ...
        isset($_GET['code']) && isset($this->_ACC[trim($_GET['code'])]) && $cate_id_now = $this->_ACC[trim($_GET['code'])]; //如果有code
        /* 取得当前分类及子孙分类cate_id */
        $cate_ids = array();
        if ($cate_id_now > 0) //排除系统内置分类
        {
    ...
    public function help() {
    	 /* 处理cate_id */
        $cate_id_now = !empty($_GET['cate_id'])? intval($_GET['cate_id']) : $this->_ACC[ACC_HELP]; //如果cate_id为空则默认显示关于我们
