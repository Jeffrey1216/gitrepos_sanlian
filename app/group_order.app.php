...
define("PAGE_NUM",20);
/*团购后台控制器*/
class Group_orderApp extends ShoppingbaseApp {
	
	public function __construct() {
		$this->Group_orderApp();
	}
	
	public function Group_orderApp() {
		parent::__construct();
	...
	public function index() {
		...
		{
			$use=$this->visitor->get('user_id');
			$user=$this->_group_order_mod->getAll("select * from pa_group_order go left join pa_group_project gp on go.project_id = gp.id left join 
			pa_group_category gc on gp.category_id = gc.id where go.status in (" . ORDER_ACCEPTED . "," . ORDER_SHIPPED . "," . ORDER_FINISHED . ") 
			and gc.id = 3 and go.buyer_id = " . $use);
			
			if($user)
			{
					$this->show_warning('尊敬的派啦网用户，你已经参与过我们的一元秒杀活动，不能再次参与！请选择其它活动参加！谢谢。');
					return;
			}
		}
		if(!IS_POST) {
			...
			$order_address = empty($_POST['order_address']) ? 0 : intval($_POST['order_address']); //收货地址
			if($order_address == 0) {//填写的收货地址
				if(trim($_POST['consignee']) == '') {
					$this->show_warning("收货人未填写!.");
					return;
				}
				if(trim($_POST['region_id']) == '') {
					$this->show_warning("收货地址未选择!.");
					return;
				}
				if(trim($_POST['address']) == '') {
					$this->show_warning("详细地址未填写!.");
					return;
				}
				if(trim($_POST['zipcode']) == '') {
					$this->show_warning("邮编未填写!.");
					return;
				}
				if(trim($_POST['phone_tel']) == '' && trim($_POST['phone_mob']) == '') {
					$this->show_warning("电话或手机未填写!.");
					return;
				}
			}
			$param = array(
			...
			header("Location:index.php?app=group_order&act=gotoPay&order_id=" . $order_id);
		}
	}
    ...
    public function gotoPay() {
		...
		if($this->checkQuantity($order_id) == -1) {
			$this->show_warning("活动不在进行或商品已售完!");
			return;
		} 
     ...
     * 	检查是否还有库存
     **/
    protected function checkQuantity($order_id) {
    	$time = time();
    	$order_info = $this->_group_order_mod->getRow("select gp.id,gp.start_time,gp.finish_time,gp.max_quantity,go.project_id from pa_group_order go 
    	left join pa_group_project gp on go.project_id = gp.id where go.order_id = " . $order_id);
    	if($order_info['max_quantity'] <= 0 || $order_info['start_time'] > $time || $order_info['finish_time'] < $time) {
    		return -1;
    	} else {
    		return 1;
    	}
