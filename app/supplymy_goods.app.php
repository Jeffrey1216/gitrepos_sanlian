...
/* 品牌商城商品管理控制器 */
class supplyMy_goodsApp extends StoreadminbaseApp
{
    var $_goods_mod;
    var $_spec_mod;
    var $_image_mod;
    ...
    {
         $this->supplyMy_goodsApp();
    }

    function supplyMy_goodsApp()
    {
        parent::__construct();
...
//      $statearr = Lang::get('status_array');
        foreach ($goods_list as $key => $goods)
        {
            $goods_list[$key]['cate_name'] = $this->_goods_mod->format_cate_name($goods['cate_name']);
            $goods_list[$key]['jprice'] = $goods['price']-$goods['credit']*0.5; //结算价 = 商品售价 - 赠送积分 × 50% 
            //$goods_list[$key]['status'] = $statearr[$goods['status']];
        }
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('goods_name','description', 'cate_id', 'cate_name', 'brand', 'spec_qty','if_show','closed','recommended')))
       {
       		...
       		if ($goods['if_show']==0)
       		{
       			if($goods['credit']==0)
       			{
       				$this->json_error('积分为0，不能上架');
       				return;
       			}elseif($goods['credit']>=$goods['price'])
       			{
       				$this->json_error('积分大于售价，不能上架');
       				return;
       			}
	       		if ($goods['cprice']<$goods['price'])
	       		{
	              	$this->json_error('市场价低于售价，不能上架');
	       			return;
	       		}
       		}
        ...
        else
        {
            ...
            foreach ($records as $record)
            {
                ...
                $goods = array(
                    ...
                    'if_show'             => 0, //导入的产品默认是下架的
                    'add_time'            => gmtime(),
                    'last_update'      => gmtime(),
                    'recommended'      => $record['recommended'],
                    'default_image' => $record['goods_image'],
                	'cprice'        => $record['price'],  //商品市场参考价
                	'price'         => $record['price'],  //商品售价
                    'closed'              => 0,
                );
    ...
    {
        return array(
            'goods_name'  => iconv('GBK','UTF-8','宝贝名称'),
            'cid'         =>  iconv('GBK','UTF-8','宝贝类目'),
            'price'       =>  iconv('GBK','UTF-8','宝贝价格'),
            'stock'       =>  iconv('GBK','UTF-8','宝贝数量'),
            'if_show'     =>  iconv('GBK','UTF-8','放入仓库'),
            'recommended' =>  iconv('GBK','UTF-8','橱窗推荐'),
            'description' =>  iconv('GBK','UTF-8','宝贝描述'),
            'goods_image' =>  iconv('GBK','UTF-8','新图片'),
            'sale_attr'   =>  iconv('GBK','UTF-8','销售属性组合'),
            'sale_attr_alias' =>  iconv('GBK','UTF-8','销售属性别名')
        );
    }
...
//        }
    ...
    function _check_post_data($data, $id = 0)
    {
        ...
        $cprice = $this->_filter_price($data['goods']['cprice']); //市场价
        $credit = $this->_filter_price($data['goods']['credit']); //赠送CC积分
