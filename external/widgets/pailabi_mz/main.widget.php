<?php
	class Pailabi_mzWidget extends BaseWidget
	{
		public $_name = 'pailabi_mz';
		public function _get_data()
		{
			...
			if($data === false)
			{
        		...
        		$pailabi_mz = $widget_mod->get(array('conditions' => "widget_name = '".$this->_name."'"));
        		$pailabi_mz_data = $pailabi_mz['widget_data'];
        		$data = unserialize($pailabi_mz_data);
				...
				$order = $goods_mod->getAll("SELECT g.default_image,g.goods_id,g.simage_url,g.yimage_url,g.mimage_url,g.smimage_url,g.dimage_url,g.goods_name,g.cprice,
												g.price,g.credit,o.seller_id,o.seller_name,o.add_time,o.buyer_name FROM pa_order_goods og left join pa_order o on
 												...
 												WHERE cate_id IN (select gc1.cate_id from pa_gcategory gc1 where gc1.parent_id in (select cate_id from pa_gcategory where parent_id = 192 or parent_id = 264 ) AND gc1.mall_type=1)) 
