 ...
 */
class Green_prefectureWidget extends BaseWidget
{
    var $_name = 'green_prefecture';
    function _get_data()
    {
    	$goods_infos = array();
        ...
        if ($data === false)
        {
            $widget_mod = &m('widget');
            $goods_mod = &m('goods');
            $green_prefecture = $widget_mod->get(array('conditions' => "widget_name = '".$this->_name."'"));
            $green_prefecture_data = $green_prefecture['widget_data'];
            $data = unserialize($green_prefecture_data);
