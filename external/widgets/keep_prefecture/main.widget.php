 ...
 */
class Keep_prefectureWidget extends BaseWidget
{
    var $_name = 'keep_prefecture';
    function _get_data()
    {
    	$goods_infos = array();
        ...
        if ($data === false)
        {
            $widget_mod = &m('widget');
            $goods_mod = &m('goods');
            $keep_prefecture = $widget_mod->get(array('conditions' => "widget_name = '".$this->_name."'"));
            $keep_prefecture_data = $keep_prefecture['widget_data'];
            $data = unserialize($keep_prefecture_data);
