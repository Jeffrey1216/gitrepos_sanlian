<?php
	class Paila_mall_jzWidget extends BaseWidget
		{
			public $_name = 'paila_mall_jz';
					...
					if($data === false)
						{
        					...
        					$paila_mall_jz = $widget_mod->get(array('conditions' => "widget_name = '".$this->_name."'"));
        					$paila_mall_jz_data = $paila_mall_jz['widget_data'];
        					$data = unserialize($paila_mall_jz_data);
							...
							//商品循环
							 $goods_ids = $data['goods'];
          					  ...
          					  foreach($goods_ids as $k => $id)
          				 	 	 {
            						$goods_info = $goods_mod->get(array('conditions'=> "goods_id={$id['goods_id']}",'fields'=>'goods_name,cprice, price,default_image,yimage_url,mimage_url,
