...
function spec_editor(){
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        var prefit_error = new Array(); //折扣率错误
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            /* 判断折扣率非法 */
            var prefitSpecStr = (Number(gprice)/Number(price)) + '';
            var prefitSpecNum = prefitSpecStr.replace(/^([0-9]+\.[0-9]{2})[0-9]*$/,"$1"); 
            if(Number(prefitSpecNum) > 0.4) {
								valid && prefit_error.push(item);
						}
        ...
        if(prefit_error.length > 0 ) {
        	var msg = lang.follow_spec_prefit_invalid + '\n';
            $.each(prefit_error,function(i,val){
                msg += val + '\n';
            });
