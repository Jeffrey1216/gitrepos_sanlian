...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            "<td class='handler'><span><a href='index.php?app=gcategory_copy&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=gcategory_copy&act=drop&id="+res[i].cate_id+"\";'>"+lang.drop+"</a> | <a href='index.php?app=gcategory_copy&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a></span></td></tr>";
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
                    change_background();
...
function secajax(ob)
{
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        if(res[i].add_child)
                        {
                            add_child =  " | <a href='index.php?app=gcategory_copy&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a>";
                        }
            ...
            "<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>"+
            "<td class='align_center'>"+if_show+"</td>"+
            "<td class='handler'><span><a href='index.php?app=gcategory_copy&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location =\"index.php?app=gcategory_copy&amp;act=drop&amp;id="+res[i].cate_id+"\";'>"+lang.drop+"</a>" + add_child + "</span></td></tr>";
                    }
                    pr.after(str);
                    change_background();
                    $('span[ectype="inline_edit"]').unbind('click');
