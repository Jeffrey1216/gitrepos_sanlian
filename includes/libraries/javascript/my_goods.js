...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        tpl.find('*[item="credit"]').append('<input type="hidden" name="credit['+ item.spec_id +']" value="' + item.credit + '" />' + item.credit);
        tpl.find('*[item="gprice"]').append('<input type="hidden" name="gprice['+ item.spec_id +']" value="' + item.gprice + '" />' + item.gprice);
...
function drop_image(goods_file_id)
{
    if (confirm(lang.uploadedfile_drop_confirm))
        {
            var url = SITE_URL + '/administr/index.php?app=goods&act=drop_image';
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
        ...
        tpl.find('*[item="credit"]').val(item.credit);
        tpl.find('*[item="gprice"]').val(item.gprice);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            var get_credit = $.trim($(this).find('*[item="credit"]').val());
            var gprice = $.trim($(this).find('*[item="gprice"]').val());
            ...
            var cprice = Number($('#cprice').val());
        	var credit = Number($('#credit').val());
        	if(cprice<price){
        		valid && price_error.push(item+lang.cprice_spec);
        	}
        	if(price<credit){
        		valid && price_error.push(item+lang.credit_spec);
        	}
            item != ';' && SPEC.specs.push({
                'spec_1':spec_1,
                'spec_2':spec_2,
                'price':number_format(price, 2),
                'credit':number_format(get_credit,2),
...
function spec_editor1(){
    ...
    // 添加规格项
    $('*[ectype="add_spec_item"]').click(function(){
        var new_spec = $('#add_sp_last').find('*[ectype="spec_item"]:last').clone(true);
        new_spec.insertAfter($('#add_sp_last').find('*[ectype="spec_item"]:last'));
        var nspec = $('*[ectype="spec_item"]:last');
        nspec.children('li').last('li').prev('li').children('span').show();
        nspec.children('li').last('li').prev('li').children('input[ectype="barcode"]').val('');
        nspec.children('li').last('li').prev('li').children('input[ectype="barcode"]').removeAttr('readOnly');
        nspec.children('li').last('li').children('span[ectype="drop_spec_item"]').show();
        hide_drop_button();
    });

    // 删除规格项
    $('*[ectype="drop_spec_item"]').click(function(){
        $('#add_sp_last').find('*[ectype="spec_item"]').length > 1 && $(this).parent().parent().remove();
        hide_drop_button();
    });

    // 规格项排序
    $('*[ectype="up_spec_item"]').click(function(){
        var prev = $(this).parent().parent().prev('*[ectype="spec_item"]').clone(true);
        $(this).parent().parent().prev('*[ectype="spec_item"]').remove();
        $(this).parent().parent().after(prev);
        hide_drop_button();
    });
    $('*[ectype="down_spec_item"]').click(function(){
        var prev = $(this).parent().parent().next('*[ectype="spec_item"]').clone(true);
        $(this).parent().parent().next('*[ectype="spec_item"]').remove();
        $(this).parent().parent().before(prev);
        hide_drop_button();
    });
}
