...
define('ORDER_REFUND',50);					   // 退款中
define('ORDER_REFUND_FINISH',60);			   // 已退款
...
define('ACC_ABOUT' , 'about');					//acategory表中code字段为about时——关于我们类别
define('ACC_ACTIVITY' , 'activity');			//acategory表中code字段为activity时——最新活动类别
define('ACC_CHANNEL' , 'channel');			    //acategory表中code字段为channel时——渠道商帮助类别
define('ACC_BRAND' , 'brand');			    //acategory表中code字段为brand时——品牌托管类别
define('ACC_CREDIT' , 'credit');			    //acategory表中code字段为brand时——品牌托管类别
define('ACC_CX' , 'cx');			    //acategory表中code字段为CX时—-派啦币商城类别
define('ACC_PCX' , 'pcx');			    //acategory表中code字段为CX时—-派啦商城类别
define('ACC_AGRO', 'agro');					//acategory表中code字段为agro时——绿色情报类别
...
EOT;
	...
	//广告缴费或会员购买大礼包 团购员返利
    function ad_manager_rebate($manager_id, $amount ,$type=0,$order_id=0) //manager_id 直接推荐的团购员id
    {
    	if ($amount>0)
    	{
	    		...
	    		if ($v['ratio']>0)
	    		{
		    		...
		    		if ($grant)
		    		{
			    	   ...
			    	   //编写收益记录
			    		...
			    		if ($type==0)
			    		{
			    			$param['change_desc'] = "团购员({$v['manager_name']})拉取广告费￥{$amount}, 获得分成比例".($v['ratio']*100)."%,￥{$grant['money']}";
			    			$param['change_type'] = 50;
			    		}else
			    		{
			    			$param['change_desc'] = "团购员({$v['manager_name']})购买团购大礼包￥{$amount}, 获得分成比例".($v['ratio']*100)."%,￥{$grant['money']}";
	    	...
	    	if ($channel_money)
	    	{
		    	...
		    	if ($type==0)
		    	{
		    		$data['change_desc'] = "团购员({$v['manager_name']})拉取广告费￥{$amount}, 渠道获得分成比例5%,￥{$channel_money['money']}";
		    		$data['change_type'] = 54;
		    	}else
		    	{
		    		$data['change_desc'] = "团购员({$v['manager_name']})购买团购大礼包￥{$amount}, 渠道获得分成比例5%,￥{$channel_money['money']}";
		    		$data['change_type'] = 55;
		    	}
    ...
    //店铺缴费,团购员返利
    function manager_rebate($manager_id, $amount, $store_id) //manager_id 直接推荐的团购员id, $store_id, 缴费店铺的id
    {
    	if ($amount>0)
    	{
	    	$_customer_manager_mod = & m('customermanager');
	    		...
	    		if ($v['ratio']>0)
	    		{
		    		...
		    		if ($grant)
		    		{
			    		...
			    		$param = array(
			    			...
			    			'user_credit' => $credit,
			    			'change_time' => gmtime(),
			    			'change_desc' => "团购员({$v['manager_name']})推荐的店铺(ID:{$_store_info['store_id']},店铺名:{$_store_info['store_name']})缴费￥{$amount}, 获得分成比例".($v['ratio']*100)."%,￥{$grant['money']}",
			    			'change_type' => 51,
			    		);
			    		add_account_log($param);
		    		}
	    	...
	    	if ($channel_money)
	    	{
		    	...
		    	$data = array(
		    		...
		    		'user_credit' => $credit,
		    		'change_time' => gmtime(),
		    	    'change_desc' => "团购员({$v['manager_name']})推荐的店铺(ID:{$_store_info['store_id']},店铺名:{$_store_info['store_name']})缴费￥{$amount}, 渠道获得分成比例5%,￥{$channel_money['money']}",
			        'change_type' => 56,
		    	);
	...
	//会员购物---所属团购员获得返利
    function mb_manager_rebate($manager_id, $credit,$order_id = 0 ,$amount) //manager_id 直接推荐的团购员id
    {
    	if ($credit>0)
    	{
	    	$grant = format_fanli_money($credit * 0.5); //用户购物所得积分的50%
	    	if ($grant)
	    	{
	    	//计算应得积分, 计算应得现金
	    	$cash = $grant['cash'];
	       ...
	       //编写收益记录
	    	$param = array(
	    		...
	    		'user_credit' => $credit,
	    		'change_time' => gmtime(),
	    		'change_desc' => "会员购物，团购员所获得的返利金额：￥{$grant['money']}",
	    		'change_type' => 53,
	    		'order_id' => $order_id,
	    	);
	    	add_account_log($param);
    ...
    function mb_channel_rebate($channel_id, $credit ,$order_id = 0 )
    {
    	if ($credit>0)
    	{
	    	$channel_money =  format_fanli_money($credit * 0.5); //用户购物所得积分的50%
	    	if ($channel_money)
	    	{
		    	//计算应得积分, 计算应得现金
		    	$cash = $channel_money['cash'];
		       ...
		       //编写收益记录
		    	$param = array(
		    		...
		    		'user_credit' => $credit,
		    		'change_time' => gmtime(),
		    		'change_desc' => "会员购物，渠道所获得的返利金额：￥{$channel_money['money']}",
		    		'change_type' => 57,
		    		'order_id' => $order_id,
		    	);
		    	add_account_log($param);
    ...
    function mb_store_rebate($store_id, $credit,$order_id )
    {
    	if ($credit>0)
    	{
	    	$balance =  format_fanli_money($credit * 0.5); //用户购物所得积分的50%
	    	if ($balance)
	    	{
		    	//计算应得积分, 计算应得现金
		    	$cash   = $balance['cash'];
		    	...
		    	$param = array(
		    		...
		    		'user_credit' => $credit,
		    		'change_time' => gmtime(),
		    		'change_desc' => "会员购物，店铺获得的返利金额：￥{$balance['money']}",
		    		'change_type' => 58,
		    		'order_id' => $order_id,
		    	);
		    	add_account_log($param);
