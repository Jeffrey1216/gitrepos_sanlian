    ...
    function view()
    {
        ...
        else
        {
            ...
            if (isset($_POST['agree']))
            {
				...
				$smscontent = "渠道已经为店铺：{$store['store_name']}定价，价格为：{$use_money}";
            ...
            elseif (isset($_POST['reject']))
            {
            	...
            	$content = get_msg('toseller_store_passed_notify');	        		
				$smscontent = "渠道已经拒绝店铺：{$store['store_name']}的定价，原因为：{$reject_reason}";
				$user_mod = & m('member');
   ...
   function get_promotion(){
     ...
     */
 	...
 	function PromotiomExecl($stock_all_info){
		...
		$objActSheet->setCellValue('J1',iconv('gbk', 'utf-8', '成本金额小计'));;
		foreach ($stock_all_info as $k => $v)
		{
			$key1=1;
    		$key2+=$key1;
	...
	function StockExcel($stock_all_info)
    {
		...
		//$objActSheet->setCellValue('I1',iconv('gbk', 'utf-8', '采购价'));
		$objActSheet->setCellValue('I1',iconv('gbk', 'utf-8', '赠送PL'));
		$objActSheet->setCellValue('J1',iconv('gbk', 'utf-8', '库存金额小计'));;
		foreach ($stock_all_info as $k => $v)
		{
			$key1=1;
    		$key2+=$key1;
	...
	private function ShipmentExcelDetail($store_order_info)
    {
		...
		$objActSheet->setCellValue('E1',iconv('gbk', 'utf-8', '批发价'));
		$objActSheet->setCellValue('F1',iconv('gbk', 'utf-8', '批发价小计'));
		$objActSheet->setCellValue('G1',iconv('gbk', 'utf-8', '派啦价'));
		$objActSheet->setCellValue('H1',iconv('gbk', 'utf-8', '派啦价小计'));
		$objActSheet->setCellValue('I1',iconv('gbk', 'utf-8', '赠送PL'));
		$objActSheet->setCellValue('J1',iconv('gbk', 'utf-8', '赠送PL小计'));
