...
/**
    ...
    * @return void
    */
function show_storeadmin_message ($msg)
{
    $a = _trigger_message(func_get_args());

    _message(serialize($a), E_USER_NOTICE,'storeadmin');
}
...
/**
    ...
    * @param string $msg
    */
function show_storeadmin_warning ($msg)
{
    $a = _trigger_message(func_get_args());

    _message(serialize($a), E_USER_WARNING,'storeadmin');
}
...
/**
    ...
    * @param   int     $type
    */
function _message($msg, $type,$info='')
{
...
class Message extends MessageBase
{
    ...
    function report_link($err, $msg, $file, $line)
    {
        if (strncmp($msg, 'MySQL Error[', 12) == 0)
        {
            ...
            $url = 'http://www.paila.com/help/faq.php?type=mysql&dberrno=' . $tmp_arr['dberrno'] . '&dberror=' .  urlencode($tmp_arr['dberror']);
    ...
    function display($info='')
    {
        ...
        else
        {
            ...
            $this->_config_seo('title', Lang::get('ecmall_sysmsg') . '-- Powered by PaiLa');
            ...
            if($info){
            	parent::display($info.'.message.html');
            }else{
