   ...
   static function startup($config = array())
    {
        ...
        /* 加载初始化文件 */
        require(ROOT_PATH . '/core/controller/app.base.php');     //基础控制器类
        require(ROOT_PATH . '/core/model/model.base.php');   //模型基础类
	...
	$cron = new Cron();
        ...
        {
            $_GET   = addslashes_deep($_GET); //addslashes_deep()函数对其参数进行递归加'\'转义
		...
		**/
		...
		if ($get_credit>0)
		{
	    	...
	    	$param = array(
	    		...
	    		'credit'  => $get_credit,
	    		'change_time' => gmtime(),
	    		'change_desc' => "系统自动操作完成订单，赠送用户积分：{$get_credit}PL",
	    		'change_type' => 1,
	    	    'order_id' => $order_id,
	    	);
 ...
 *
 *    @author    lihuoliang
 *    @parama	 string $file_path  文件的相当路径
 *               string $dirname    文件根目录路径
 *    @return    boolen
 */
